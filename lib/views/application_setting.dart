...
class HideOnMinimizeItem extends ConsumerWidget {
  const HideOnMinimizeItem({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final hideOnMinimize = ref.watch(
      appSettingProvider.select((state) => state.hideOnMinimize),
    );
    return ListItem.switchItem(
      title: Text(appLocalizations.hideOnMinimize),
      subtitle: Text(appLocalizations.hideOnMinimizeDesc),
      delegate: SwitchDelegate(
        value: hideOnMinimize,
        onChanged: (bool value) {
          ref
              .read(appSettingProvider.notifier)
              .update((state) => state.copyWith(hideOnMinimize: value));
        },
      ),
    );
  }
